...
int main(int argc, char *argv[]){
    ...
    for (int i = 0; i < 12;i++){
        ...
        for(int i = 0; i < warmup_epoch;i++) {
            if (rank == 0) {
                MPI_Isend(ptr, alloc_size, MPI_DOUBLE, 1, node_tag1, MPI_COMM_WORLD, &send_request);
                MPI_Irecv(ptr, alloc_size, MPI_DOUBLE, 1, node_tag2, MPI_COMM_WORLD, &recv_request);
            } else {
                MPI_Irecv(ptr, alloc_size, MPI_DOUBLE, 0, node_tag1, MPI_COMM_WORLD, &send_request);
                MPI_Isend(ptr, alloc_size, MPI_DOUBLE, 0, node_tag2, MPI_COMM_WORLD, &recv_request);
            }
        }
        ...
        for(int i = 0; i < iterations;i++) {
            if (rank == 0) {
                MPI_Isend(ptr, alloc_size, MPI_DOUBLE, 1, node_tag1, MPI_COMM_WORLD);
                MPI_Irecv(ptr, alloc_size, MPI_DOUBLE, 1, node_tag2, MPI_COMM_WORLD, &stat);
            } else {
                MPI_Irecv(ptr, alloc_size, MPI_DOUBLE, 0, node_tag1, MPI_COMM_WORLD, &stat);
                MPI_Isend(ptr, alloc_size, MPI_DOUBLE, 0, node_tag2, MPI_COMM_WORLD);
            }
        }
